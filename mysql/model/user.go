...
type User struct {
	Model
	Username string `gorm:"type:varchar(255);not null;uniqueIndex;comment:用户名" json:"username"`
	Password string `gorm:"type:varchar(255);not null;comment:密码" json:"password,omitempty"`
	Email    string `gorm:"type:varchar(255);not null;index;comment:电子邮件" json:"email"`
